...
class Audio:
    ...
    def do_record(self):
        ...
        while True:
            try:
                ...
                Audio.speak(self,"what name will you give it?")
                name_file = Ai_Listening().name()
                # wants to confirm if you want to name it as
                Audio.speak(self,"are you sure you want to save it as '" + name_file + "'")
                print("Yes                          No")
    ...
    def play_file(self):
        Audio.speak(self,"which file should I play?")
        try:
...
class Sites_and_Apps:
    ...
    def search(self, search_value, do_open=True):
    ...
    def open_site(self, site_name):
        val = Sites_and_Apps.open_app(self,site_name)
        if val != None:
            os.startfile(val)
        else:
    ...
    def open_app(self, word):
        apps = {"word": "WINWORD.EXE", "powerpoint": "POWERPNT.EXE", "notepad": "notepad.exe", "excel": "EXCEL.EXE",
                "python": "python.exe", "visual studio": "devenv.exe",
                "steam": "C:\\Users\\gall_\\Desktop\\Steam\\steam.exe"}
        for k in apps.keys():
            if k in word:
                return apps[k]
        return None
    ...
    def do_open(self, text):
        ...
        if text == "google":
            ...
            return None
        Sites_and_Apps.open_site(self,text)
        # says he is opening that
        Audio().speak("opening " + text)

    def do_search(self,text):
        text2 = text.lower().replace("search", "").replace("luigi", "")
        Sites_and_Apps.search(self,text2)
        Audio().speak("searching " + text2)

    def wiki(self,text):
        try:
            fixed = wikipedia.suggest(text)
            if fixed is None:
                fixed = text
            print(wikipedia.summary(fixed, sentences=5))
        except wikipedia.exceptions.DisambiguationError:
            print(f"{fixed} may refer to more than one option. Be more specific")

    def play_videos(self, text):
        try:
    ...
    def questions_and_answers(self, question):
        # App id obtained by the above steps
        try:
            app_id = "JQYK6E-H2878ETPU7"
            ...
            print("Something went wrong. Try again or ask a different question")
